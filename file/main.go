...
import (
	...
	"github.com/autobar-dev/services/file/routes"
...
func main() {
	...
	minio_client, err := minio.New(config.S3Endpoint, &minio.Options{
	...
	app_context := &types.AppContext{
		...
		Repositories: &types.Repositories{
			...
			File:  repositories.NewFileRepository(database),
			S3:    repositories.NewS3Repository(minio_client, config.S3BucketName),
		},
	}
