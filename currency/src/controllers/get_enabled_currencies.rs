...
pub async fn get_enabled_currencies_controller(context: app_context::Context) -> Result<Vec<types::EnabledCurrency>, RestError> {
  ...
  let mut enabled_currencies_response: Vec<types::EnabledCurrency> = Vec::new();

  for enabled_currency in enabled_currencies {
    enabled_currencies_response.push(types::EnabledCurrency::from(enabled_currency));
  }
